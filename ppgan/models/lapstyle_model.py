...
class LapStyleDraThumbModel(BaseModel):
    ...
    def backward_Dec(self):
        ...
        self.loss = self.loss_ps * self.style_weight *.55 + self.loss_content_p * self.content_weight*.4 +\
                    self.loss_patch * self.content_weight * 7 +\
...
class LapStyleRevFirstThumb(BaseModel):
    ...
    def backward_G(self, optimizer):
        ...
        self.patch_loss = self.loss_ps * self.style_weight *.55 + self.loss_content_p * self.content_weight*.4 +\
    ...
    def backward_Dpatch(self):
        ...
        self.loss_Dp_real = 0
        transformed = paddle.slice(self.sp,axes=[2,3],starts=[self.position[0],self.position[2]],ends=[self.position[1],self.position[3]])
        pred_Dp_real = self.nets['netD_patch'](transformed.detach())
