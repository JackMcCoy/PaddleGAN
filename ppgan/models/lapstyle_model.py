...
class LapStyleRevSecondMiddle(BaseModel):
    ...
    def backward_G(self,i):
        ...
        style_counter=0
        if type(self.cX)==bool:
            self.cX = xdog(self.content.detach(),self.gaussian_filter,self.gaussian_filter_2,self.morph_conv_2,morph_cutoff=400,morphs=1)
            self.sX = xdog(self.style_stack[1].detach(),self.gaussian_filter,self.gaussian_filter_2,self.morph_conv_2,morph_cutoff=400,morphs=1)
        cX = self.cX
        sX = self.sX
        for j in range(i+1):
            cX = paddle.slice(self.cX,axes=[2,3],starts=[self.positions[j][1].astype('int32'),self.positions[j][0].astype('int32')],ends=[self.positions[j][3].astype('int32'),self.positions[j][2].astype('int32')])
