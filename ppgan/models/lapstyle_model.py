...
class LapStyleDraThumbModel(BaseModel):
    ...
    def forward(self):
        ...
        self.stylized_thumb = self.nets['net_dec'](self.cF, self.cpF, self.sF,  'thumb')
        self.stylized_patch = self.nets['net_dec'](self.cF, self.cpF, self.sF, 'patch')
    ...
    def backward_Dec(self):
        with paddle.no_grad():
            ...
            g_t_thumb_crop = paddle.slice(g_t_thumb_up,axes=2,starts=[self.position[0],self.position[2]],ends=[self.position[1],self.position[3]])
        ...
        for layer in self.content_layers:
            self.l_identity2 += self.calc_content_loss(self.Fcc[layer],
                                                       self.cF[layer])
        self.Ipcc = self.nets['net_dec'](self.cpF, self.cpF)
        ...
        self.loss_patch_content_relt = self.calc_content_relt_loss(
            self.tpF['r31'], self.cpF['r31']) + self.calc_content_relt_loss(
                self.tpF['r41'], self.cpF['r41'])
        self.losses['loss_style_remd'] = self.loss_style_remd
        self.losses['loss_content_relt'] = self.loss_content_relt
        self.losses['loss_patch_content_relt'] =  self.loss_patch_content_relt

        self.loss = self.loss_c * self.content_weight + self.loss_s * self.style_weight +\
                    self.loss_patch * self.content_weight +\
                    self.l_identity1 * 50 + self.l_identity3 *50 +\
                    self.l_identity2 * 1 + self.l_identity4 *1 + self.loss_style_remd * 10 + \
                    self.loss_content_relt * 16
        self.loss.backward()
