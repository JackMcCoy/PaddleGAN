...
class LapStyleDraThumbModel(BaseModel):
    ...
    def backward_Dec(self):
        ...
        self.loss = self.loss_s * self.style_weight +\
                    ...
                    self.loss_content_relt * 24 +self.loss_patch * self.content_weight
        self.loss.backward()
...
class LapStyleRevSecondPatch(BaseModel):
    ...
    def forward(self):
        ...
        revnet_input = paddle.concat(x=[self.pyr_ci[0], stylized_up], axis=1)
        stylized_rev_lap_second,_ = self.nets['net_rev_2'](revnet_input.detach())
        ...
        stylized_rev_patch,_ = self.nets['net_rev_2'](revnet_input.detach())
    ...
    def backward_G_p(self):
        transformed = paddle.slice(self.sp, axes=[2, 3], starts=[self.position[0], self.position[2]],
                                   ends=[self.position[1], self.position[3]])
        spCrop = self.nets['net_enc'](transformed)
        tpF = self.nets['net_enc'](self.p_stylized)
        cpF = self.nets['net_enc'](self.cp)
        with paddle.no_grad():
            tt_cropF = self.nets['net_enc'](self.thumb_crop)
        """patch loss"""
        loss_patch = 0
        # self.loss_patch= self.calc_content_loss(self.tpF['r41'],self.tt_cropF['r41'])#+\
        #                self.calc_content_loss(self.tpF['r51'],self.tt_cropF['r51'])
        ...
        p_loss_style_remd = self.calc_style_emd_loss(
            tpF['r31'], spCrop['r31']) + self.calc_style_emd_loss(
            tpF['r41'], spCrop['r41'])
        p_loss_content_relt = self.calc_content_relt_loss(
            tpF['r31'], cpF['r31']) + self.calc_content_relt_loss(
            tpF['r41'], cpF['r41'])
...
class LapStyleRevFirstPatch(BaseModel):
    ...
    def forward(self):
        ...
        stylized_med = F.interpolate(stylized_small, scale_factor=8)
        stylized_rev_lap = F.interpolate(stylized_rev_lap, scale_factor=4)
        stylized_rev_lap = paddle.slice(stylized_rev_lap,axes=[2,3],starts=[self.half_position[0],self.half_position[2]],ends=[self.half_position[1],self.half_position[3]])
        stylized_rev_lap = paddle.slice(stylized_rev_lap,axes=[2,3],starts=[i[0],i[2]],ends=[i[1],i[3]])
        stylized_med = paddle.slice(stylized_med,axes=[2,3],starts=[self.position[0],self.position[2]],ends=[self.position[1],self.position[3]])
        ...
        p_stylized_rev_patch = fold_laplace_pyramid([p_stylized_rev_patch, p_stylized_rev_lap,stylized_rev_lap,sylized_med])
    ...
    def backward_Dpatch(self):
        ...
        self.loss_Dp_real = self.nets['netD_patch'](self.style_patch.detach())
        pred_Dp_real += self.gan_criterion(self.loss_Dp_real, True)
