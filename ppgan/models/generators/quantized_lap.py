...
class VectorQuantize(nn.Layer):
    def __init__(
        ...
        if transformer_size==1:
            ...
            self.pos_embedding = paddle.create_parameter(paddle.ParamAttr(),shape=(1, 256, 512), dtype='float32', is_bias=True)
        elif transformer_size==2:
            self.transformer = Transformer(256, 8, 16, 64, 256, dropout=0.1)
            self.pos_embedding = paddle.create_parameter(paddle.ParamAttr(),shape=(1, 1024, 256), dtype='float32', is_bias=True)
        elif transformer_size==3:
            self.transformer = Transformer(2048, 8, 16, 64, 768, dropout=0.1)
            self.pos_embedding = paddle.create_parameter(paddle.ParamAttr(), shape=(1, 256, 2048), dtype='float32', is_bias=True)
    @property
    def codebook(self):
