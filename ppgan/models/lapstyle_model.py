...
class LapStyleRevFirstThumb(BaseModel):
    ...
    def backward_Dpatch(self):
        ...
        pred_Dp_real = 0
        reshaped = paddle.slice(self.sp, starts=[self.position[0],self.position[2]],ends=[self.positions[1],self.positions[3]])
...
class LapStyleRevSecondPatch(BaseModel):
    ...
    def backward_G(self):
        ...
        self.loss = self.loss_Gp_GAN +self.loss_ps/4 * self.style_weight +\
                    self.loss_content_p * self.content_weight +\
                    self.loss_patch *5 * self.content_weight +\
    ...
    def backward_G_p(self):
        ...
        loss_patch = loss_Gp_GAN +loss_ps/4 * self.style_weight +\
                    loss_content_p * self.content_weight +\
                    loss_patch *5* self.content_weight +\
