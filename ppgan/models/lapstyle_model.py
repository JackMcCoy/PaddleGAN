...
def xdog(im, g, g2,morph_conv,gamma=.98, phi=100, eps=-.001  , k=1.6):
    ...
    for i in range(im.shape[1]):
        ...
        for j in range(im.shape[0]):
            ...
            morphed[j,i,:,:]= paddle.zeros_like(morphed[j,i,:,:])+(imdiff[j,i,:,:] > mean).astype('float32')*(morphed[j,i,:,:]>=.55).astype('float32')
    return morphed
...
class LapStyleDraXDOG(BaseModel):
    def __init__(self,
        ...
        self.morph_conv = paddle.nn.Conv2D(1,1,3,padding=1,groups=1,
                                           ...
                                           weight_attr = paddle.ParamAttr(
                                        initializer=paddle.fluid.initializer.NumpyArrayInitializer(
                                                        value=gaussian(3, 1).numpy()), trainable=False)
                                    )
