...
class LapStyleRevFirstThumb(BaseModel):
    ...
    def backward_G(self, optimizer):
        ...
        self.loss = self.loss_Gp_GAN +self.loss_ps * self.style_weight *1.25 +\
                          self.loss_content_p * self.content_weight +\
                    self.loss_content_p * self.content_weight +\
                    self.loss_patch * self.content_weight * 20 +\
                    self.p_loss_style_remd * 16 + self.p_loss_content_relt * 16
        self.loss.backward()
...
class LapStyleRevSecondPatch(BaseModel):
    ...
    def test_iter(self, output_dir=None,metrics=None):
        ...
        with paddle.no_grad():
            ...
            for i in range(0,size_x-self.in_size_x+1,move_x):
                print('i='+str(i))
                for j in range(0,size_y-self.in_size_y+1,move_y):
                    print(str(i)+', '+str(j))
                    self.outer_loop=(i,j)
    ...
    def test_forward(self):
        ...
        for i in range(0,size_x-in_size_x+1,move_x):
            for j in range(0,size_y-in_size_y+1,move_y):
                if self.outer_loop[0]*8+(i+in_size_x)*4>self.content.shape[-2] or self.outer_loop[1]*8+(j+in_size_y)*4>self.content.shape[-1]:
                    continue
                ...
                lap_2 = paddle.slice(self.laplacians[2],axes=[2,3],starts=[self.outer_loop[0]*4+i*2,self.outer_loop[1]*4+j*2],
                                   ends=[self.outer_loop[0]*2+i+in_size_x,self.outer_loop[1]*4+j*2+in_size_y])
                ...
                for k in range(0,size_x-in_size_x+1,move_x):
                    for l in range(0,size_y-in_size_y+1,move_y):
                        if self.outer_loop[0]*8+i*4+k*2+in_size_x>self.content.shape[-2] or self.outer_loop[1]*8+j*4+l*2+in_size_y>self.content.shape[-1]:
                            continue
    ...
    def backward_G(self):
        ...
        self.loss = self.loss_Gp_GAN +self.loss_ps/4 * self.style_weight +\
                    ...
                    self.p_loss_style_remd/4 * 16 + self.p_loss_content_relt * 16
        self.loss.backward()
