...
def xdog(im, g, g2,morph_conv,gamma=.99, phi=200, eps=-.05, k=1.6):
    ...
    print('mean imdiff='+str(imdiff.mean()))
    ...
    for i in range(im.shape[1]):
        ...
        for j in range(im.shape[0]):
            ...
            morphed[j,i,:,:]= paddle.zeros_like(morphed[j,i,:,:])+(imdiff[j,i,:,:] > mean).astype('float32')*(morphed[j,i,:,:]>=.65).astype('float32')
    return morphed
...
class LapStyleDraXDOG(BaseModel):
    def __init__(self,
        ...
        self.morph_conv = paddle.nn.Conv2D(1,1,5,padding=2,groups=1,
                                           ...
                                           weight_attr = paddle.ParamAttr(
                                        initializer=paddle.fluid.initializer.NumpyArrayInitializer(value=gaussian(5,1.5).numpy()), trainable=False)
                                    )
