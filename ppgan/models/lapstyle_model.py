...
class LapStyleRevFirstMXDOG(BaseModel):
    ...
    def backward_G(self):
        ...
        self.loss = self.loss_G_GAN + self.loss_c * self.content_weight + self.loss_s * self.style_weight +\
                    self.loss_style_remd * 16 + self.loss_content_relt * 16 + mxdogloss
        self.loss.backward()
        return self.loss
...
class LapStyleRevSecondPatch(BaseModel):
    ...
    def test_iter(self, output_dir=None,metrics=None):
        ...
        for i in [.25,.5,1]:
            if i==1:
                self.laplacians.append(laplacian_conv(self.content,self.lap_filter))
            else:
                self.laplacians.append(laplacian_conv(F.interpolate(self.content, scale_factor=i),self.lap_filter))
        with paddle.no_grad():
    ...
    def backward_G(self):
        ...
        if self.use_mdog==1:
            self.cX = xdog(self.content_stack[2].detach(),self.gaussian_filter,self.gaussian_filter_2,self.morph_conv,morph_cutoff=self.morph_cutoff,morphs=2)
    ...
    def backward_G_p(self):
        ...
        if self.use_mdog==1:
            self.cX = F.interpolate(self.cX, scale_factor=2)
            self.cX = crop_upsized(self.cX,self.positions[2],self.size_stack[2])
            self.cX = xdog(self.cX,self.gaussian_filter,self.gaussian_filter_2,self.morph_conv,morph_cutoff=self.morph_cutoff,morphs=2)
            cXF = self.nets['net_enc'](self.cX)
        ...
        loss_patch = loss_Gp_GAN * self.gan_patch_weight +loss_ps/4 * self.style_weight +\
                    loss_content_p * self.content_weight +\
                    loss_patch +\
                    p_loss_style_remd/4 *26 + p_loss_content_relt * 26 +mxdogloss
        loss_patch.backward()
...
class LapStyleRevSecondMXDOG(BaseModel):
    ...
    def setup_input(self, input):
        ...
        if self.is_train:
            self.content_stack = []
            self.style_stack = [paddle.to_tensor(input['style_stack_1']),paddle.to_tensor(input['style_stack_2']),paddle.to_tensor(input['style_stack_3']),paddle.to_tensor(input['style_stack_4'])]
            self.laplacians=[]
            for i in range(1,6):
                if 'content_stack_'+str(i) in input:
                    self.content_stack.append(paddle.to_tensor(input['content_stack_'+str(i)]))
    ...
    def backward_G(self,i):
        ...
        for j in range(i+1):
            ...
            sX = paddle.slice(self.sX,axes=[2,3],starts=[self.positions[j][1].astype('int32')*2,self.positions[j][0].astype('int32')*2],ends=[self.positions[j][3].astype('int32')*2,self.positions[j][2].astype('int32')*2])
        ...
        self.loss = self.loss_Gp_GAN *(self.gan_thumb_weight+(i*.6)) +self.loss_ps/4 * self.style_weight +\
                    self.loss_content_p * self.content_weight +\
                    self.loss_patch * (2.3-i*.6) +\
                    self.p_loss_style_remd/4 * 18 + self.p_loss_content_relt * 18 + mxdogloss
        self.loss.backward()
    ...
    def backward_D(self,i):
        ...
        loss_Dp_fake = paddle.clip(self.gan_criterion(pred_p_fake, False), 1e-5, 1e5)
        ...
        for k in reshaped:
            for j in paddle.split(k, 2, 3):
                ...
                pred_Dp_real += paddle.clip(self.gan_criterion(loss_Dp_real, True), 1e-5, 1e5)
