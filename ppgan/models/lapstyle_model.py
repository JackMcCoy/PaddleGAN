...
class LapStyleRevSecondMXDOG(BaseModel):
    def __init__(self,
        ...
        self.gaussian_filter_2 = paddle.nn.Conv2D(3, 3, 23,
                                                  groups=3, bias_attr=False,
                                                  padding=11, padding_mode='reflect',
                                                  weight_attr=paddle.ParamAttr(
                                                      initializer=paddle.fluid.initializer.NumpyArrayInitializer(
                                                          value=g2), trainable=False)
                                                  )
    ...
    def backward_G(self,i):
        ...
        self.loss = self.loss_Gp_GAN*((i+1.8) * self.gan_thumb_weight) +self.loss_ps/4 * self.style_weight*(2**(.5+i)) +\
                    self.loss_content_p * self.content_weight +\
                    self.loss_patch +\
                    self.p_loss_style_remd/4 * 16*(2**(.5+i)) + self.p_loss_content_relt * 16*(2**(.5+i)) + mxdogloss
        self.loss.backward()
