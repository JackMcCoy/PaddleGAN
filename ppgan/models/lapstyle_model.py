...
def xdog(im, g, g2,morph_conv,gamma=.94, phi=50, eps=-.1, diff=False, position=False):
    ...
    imdiff -= paddle.expand_as(imdiff.min(axis=[0,1,2],keepdim=True),imdiff)
    immax = paddle.expand_as(imdiff.max(axis=[0,1,2],keepdim=True),imdiff)
    imdiff /= immax
    ...
    print(paddle.expand_as(morphed.max(axis=[0,1,2],keepdim=True)),morphed)
    passedlow= (imdiff>= mean).astype('float32')*(morphed>= 70).astype('float32')
