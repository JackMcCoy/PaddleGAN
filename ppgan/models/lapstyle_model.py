...
class LapStyleRevSecondMXDOG(BaseModel):
    ...
    def backward_G(self,i):
        ...
        self.loss = self.loss_Gp_GAN*((i+1.6) * self.gan_thumb_weight) +self.loss_ps/4 * self.style_weight +\
                    self.loss_content_p * self.content_weight +\
                    self.loss_patch +\
                    self.p_loss_style_remd/4 * 26 + self.p_loss_content_relt * 26 + mxdogloss
        self.loss.backward()
