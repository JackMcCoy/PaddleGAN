...
class LapStyleDraThumbModel(BaseModel):
    ...
    def backward_Dec(self):
        ...
        """content loss"""
        self.loss_c = self.calc_content_loss(self.ttF[self.content_layers[-1]],
                                                  self.cF[self.content_layers[-1]],
                                                  norm=True)
        self.losses['loss_c'] = self.loss_c
        """patch loss"""
        self.loss_patch = 0
        ...
        for layer in self.content_layers:
            self.l_identity2 += self.calc_content_loss(self.Fcc[layer],
                                                       self.cF[layer])
        self.Ipcc = self.nets['net_dec'](self.cpF, self.cpF, self.cpF,'thumb')
        ...
        self.loss = self.loss_c * self.content_weight + self.loss_s * self.style_weight +\
                    ...
                    self.l_identity1 * 25 + self.l_identity3 *25 +\
                    self.l_identity2 * .5 + self.l_identity4 *.5 + self.loss_style_remd * 10 + \
                    self.loss_content_relt * 16
        self.loss.backward()
