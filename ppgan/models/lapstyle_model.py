...
class LapStyleRevSecondMXDOG(BaseModel):
    ...
    def backward_G(self,i):
            ...
            for idx,k in enumerate(reshaped):
                for itx,j in enumerate(paddle.split(k, 2, 3)):
                    ...
                    self.p_loss_style_remd += self.calc_style_emd_loss(
                        tpF['r31'], spF['r31']) + self.calc_style_emd_loss(
                        tpF['r41'], spF['r41'])
                    sX,_ = xdog(j.detach(),self.gaussian_filter,self.gaussian_filter_2,self.morph_conv,morphs=morph_num,minmax=sxminmax)
                    sXF = self.nets['net_enc'](sX)
                    mxdog_style+=self.calc_style_loss(cdogF['r31'], sXF['r31'])
                    style_counter += 1
            self.loss_ps = self.loss_ps/4
            self.p_loss_style_remd=self.p_loss_style_remd/4
            mxdog_style=mxdog_style/4
        else:
        ...
        mxdogloss=mxdog_content * .01 + mxdog_content_contraint *50 + mxdog_style * 51.5
    ...
    def train_iter(self, optimizers=None):
        ...
        self.forward()
        # update D
        self.set_requires_grad(self.nets['netD_1'], True)
        self.optimizers['optimD1'].clear_grad()
        optimlosses=[]
        for i in range(4):
            optimlosses.append(self.backward_D(self.nets['netD_1'],i,str(i)))
        (optimlosses[0]+optimlosses[1]+optimlosses[2]+optimlosses[3]).backward()
        self.optimizers['optimD1'].step()
        self.set_requires_grad(self.nets['netD_1'], False)
        optimizers['optimD1'].clear_grad()
        optimizers['optimG'].clear_grad()
        g_losses=[]
        # update G
        for i in range(4):
