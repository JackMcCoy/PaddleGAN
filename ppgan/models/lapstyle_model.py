...
class LapStyleDraThumbModel(BaseModel):
    ...
    def backward_Dec(self):
        ...
        for layer in self.content_layers:
            self.l_identity2 += self.calc_content_loss(self.Fcc[layer],
                                                       self.cF[layer])
        self.Ipcc = self.nets['net_dec'](self.cF, self.cF, self.cpF, 'patch')
        ...
        self.losses['loss_style_remd'] = self.loss_style_remd
        self.losses['loss_content_relt'] = self.loss_content_relt
        self.loss_patch_content =  self.calc_content_relt_loss(self.tpF['r31'], self.cpF['r31'])+self.calc_content_relt_loss(self.tpF['r41'], self.cpF['r41'])
        #self.loss_style_patch_remd = self.calc_style_emd_loss(self.tpF['r41'], self.spCrop['r41'])

        self.loss = self.loss_c * self.content_weight + self.loss_s * self.style_weight +\
                    self.loss_patch * self.content_weight +\
                    self.l_identity1 * 50 + self.l_identity2 * 1 +self.l_identity3 * 50 + self.l_identity4 * 1 +\
                    self.loss_content_relt * 16 + self.loss_patch_content*10# + self.loss_style_patch_remd*16
        self.loss.backward()
