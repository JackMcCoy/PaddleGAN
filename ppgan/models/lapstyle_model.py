...
def xdog(im, g, g2,morph_conv,gamma=.96, phi=200, eps=-.1, k=1.6):
    ...
    for i in range(im.shape[1]):
        ...
        for j in range(im.shape[0]):
            ...
            morphed[j,i,:,:]= paddle.zeros_like(morphed[j,i,:,:])+(imdiff[j,i,:,:] > mean).astype('float32')*(morphed[j,i,:,:]>=.9).astype('float32')
    return morphed
