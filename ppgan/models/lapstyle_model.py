...
class LapStyleDraXDOG(BaseModel):
    ...
    def backward_Dec(self):
        ...
        self.cX,_ = xdog(self.ci.detach(),self.gaussian_filter,self.gaussian_filter_2,self.morph_conv,gamma=.99,morph_cutoff=self.morph_cutoff,morphs=2)
        self.sX,_ = xdog(self.si.detach(),self.gaussian_filter,self.gaussian_filter_2,self.morph_conv,gamma=.99,morph_cutoff=self.morph_cutoff,morphs=2)
        ...
        stylized_dog,_ = xdog(self.stylized,self.gaussian_filter,self.gaussian_filter_2,self.morph_conv,gamma=.99,morph_cutoff=self.morph_cutoff,morphs=2)
        ...
        self.loss = self.loss_c * self.content_weight + self.loss_s * self.style_weight +\
                    ...
                    self.loss_content_relt * 16 +self.loss_style_remd * 28
        self.loss.backward()
...
class LapStyleRevSecondMXDOG(BaseModel):
    ...
    def backward_Dec(self):
        ...
        stylized_dog,_ = xdog(self.stylized[0],self.gaussian_filter,self.gaussian_filter_2,self.morph_conv,morph_cutoff=self.morph_cutoff,morphs=2)
        ...
        self.loss = self.loss_c * self.content_weight + self.loss_s * self.style_weight * 2 +\
                    self.l_identity1 * 50 + self.l_identity2 * 1 + \
                    mxdog_content * .0125 + mxdog_content_contraint *25 + mxdog_content_img * 125+\
