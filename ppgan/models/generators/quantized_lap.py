...
class VectorQuantize(nn.Layer):
    def __init__(
        ...
        if transformer_size==1:
            self.transformer = nn.Sequential(*[ImageLinearAttention(512, kernel_size = 1, padding = 0, stride = 1, key_dim = 16, value_dim = 16, heads = 8, norm_queries = False),nn.GELU()] * 6)
            ...
            self.transformer = nn.Sequential(*[ImageLinearAttention(256, kernel_size = 1, padding = 0, stride = 1, key_dim = 32, value_dim = 32, heads = 8, norm_queries = False),nn.GELU()] * 4)
            ...
            self.transformer = nn.Sequential(*[ImageLinearAttention(128, kernel_size = 1, padding = 0, stride = 1, key_dim = 64, value_dim = 64, heads = 8, norm_queries = False),nn.GELU()] * 4)
