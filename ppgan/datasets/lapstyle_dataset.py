...
class LapStyleThumbset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if small_edge=='height':
            ...
            bottommost=0
            leftmost= random.choice(list(range(0, intermediate_width*2 - self.thumb_size*2,2)))
            rightmost=leftmost+self.thumb_size*2
        else:
            rightmost=content_patches.shape[0]
            leftmost=0
            bottommost = random.choice(list(range(0, intermediate_height*2 - self.thumb_size*2,2)))
