...
class LapStyleDraXDOG(BaseModel):
    def __init__(self,
        ...
        self.gaussian_filter_2.set_state_dict({'weight':gaussian(10,.6*1.8)})
        self.morph_conv = paddle.nn.Conv2D(3,3,5,padding=2,groups=1,padding_mode='reflect',weight_attr=paddle.ParamAttr(initializer=paddle.nn.initializer.Bilinear(),trainable=False),bias_attr=False)
        self.gaussian_filter_2.set_state_dict({'weight':gaussian(5,.5)})
