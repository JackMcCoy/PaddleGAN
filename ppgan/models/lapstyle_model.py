...
def xdog(im, g, g2,morph_conv,gamma=.94, phi=50, eps=-.1, diff=False, position=False):
    ...
    passedlow= paddle.multiply((imdiff>= mean).astype('float32'),(morphed>= .999).astype('float32'))
...
class LapStyleDraXDOG(BaseModel):
    def __init__(self,
        ...
        self.morph_conv = paddle.nn.Conv2D(3,3,7,padding=3,groups=3,
                                           ...
                                           weight_attr = paddle.ParamAttr(
                                        initializer=paddle.fluid.initializer.NumpyArrayInitializer(value=np.repeat(gaussian(7, 1).numpy(),3,axis=0)), trainable=False)
                                    )
        print(gaussian(7, 1))
