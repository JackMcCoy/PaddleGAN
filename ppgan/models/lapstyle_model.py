        ...
        with paddle.no_grad():
            ...
            for a,b,c in zip(style_paths,positions,set_letter):
                with Image.open(os.path.join(self.output_dir, 'visual_test','tiles',a)) as file:
                    ...
                    tiles_1[b[0]:b[0]+image.shape[0],b[1]:b[1]+image.shape[1],0] = image[:,:,0]*(k-(1/w)) +\
                                (tiles_1[b[0]:b[0]+image.shape[0],b[1]:b[1]+image.shape[1],0]*(1-k+(w/w+1)))
                    tiles_1[b[0]:b[0] + image.shape[0], b[1]:b[1] + image.shape[1], 1] = image[:, :, 1] * (k-(1/w)) + (
                                tiles_1[b[0]:b[0] + image.shape[0], b[1]:b[1] + image.shape[1], 1] * (1 - k+(w/w+1)))
                    tiles_1[b[0]:b[0] + image.shape[0], b[1]:b[1] + image.shape[1], 2] = image[:, :, 2] * (k-(1/w)) + (
                                tiles_1[b[0]:b[0] + image.shape[0], b[1]:b[1] + image.shape[1], 2] * (1 - k+(w/w+1)))
                    not_visited[b[0]:b[0]+image.shape[0],b[1]:b[1]+image.shape[1]]=1
                    weights[b[0]:b[0] + image.shape[0], b[1]:b[1] + image.shape[1]] = weights[b[0]:b[0]+image.shape[0],b[1]:b[1]+image.shape[1]]+(k-(1/w))
