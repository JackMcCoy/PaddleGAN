...
class LapStyleThumbsetInference(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.patch_depth):
            bottommost = random.choice(list(range(0, content_img.shape[1] - self.content_thumb.shape[1],2)))
            topmost=bottommost+content_thumb.shape[1]
            leftmost = random.choice(list(range(0, content_img.shape[2] - self.content_thumb.shape[2],2)))
