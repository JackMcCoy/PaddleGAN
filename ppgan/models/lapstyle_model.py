...
def xdog(im, g, g2,morph_conv,gamma=.96, phi=100, eps=-.05, k=1.6):
...
class LapStyleDraXDOG(BaseModel):
    def __init__(self,
        ...
        self.gaussian_filter_2 = paddle.nn.Conv2D(1, 1,9,
                                groups=1, bias_attr=False,
                                padding=4, padding_mode='reflect')
        self.gaussian_filter.set_state_dict({'weight':gaussian(9,1)})
        self.gaussian_filter_2.set_state_dict({'weight':gaussian(9,1*1.6)})
        self.morph_conv = paddle.nn.Conv2D(1,1,7,padding=4,groups=1,padding_mode='reflect',bias_attr=False)
    ...
    def backward_Dec(self):
        ...
        self.loss = self.loss_c * self.content_weight + self.loss_s * self.style_weight +\
                    self.l_identity1 * 150 + self.l_identity2 * 3 + self.loss_style_remd * 40 + \
                    self.loss_content_relt * 48 + \
