...
def xdog(im, g, g2,morph_conv,gamma=.92, phi=300, eps=-.25, k=1.6):
...
class LapStyleDraXDOG(BaseModel):
    def __init__(self,
        ...
        self.morph_conv = paddle.nn.Conv2D(1,1,9,padding=4,groups=1,
                                           ...
                                           weight_attr=paddle.ParamAttr(
                                               initializer=paddle.fluid.initializer.NumpyArrayInitializer(value=gaussian(9,1.05).numpy()),trainable=False)
                                           )
