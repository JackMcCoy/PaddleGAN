...
class LapStyleRevFirstThumb(BaseModel):
    ...
    def forward(self):
        ...
        stylized_rev_lap,stylized_feats = self.nets['net_rev'](revnet_input.detach(),stylized_feats.detach(),lap_stylized_feats)
    ...
    def backward_G(self, optimizer):
        ...
        self.loss = self.loss_Gp_GAN+self.loss_ps *1.5 * self.style_weight*1.75 +\
                    ...
                    self.loss_content_p * self.content_weight +\
                    self.loss_patch * self.content_weight*.4 +\
                    self.p_loss_style_remd * 20 + self.p_loss_content_relt * 26
        self.loss.backward()
