...
class LapStyleDraXDOG(BaseModel):
    def __init__(self,
        ...
        self.gaussian_filter_2 = paddle.nn.Conv2D(1, 1,31,
                                groups=1, bias_attr=False,
                                padding=15, padding_mode='reflect',
                                weight_attr = paddle.ParamAttr(
                                        initializer=paddle.fluid.initializer.NumpyArrayInitializer(value=gaussian(31, 5).numpy()), trainable=False)
                                    )
...
class LapStyleRevFirstThumb(BaseModel):
    def __init__(self,
        ...
        if self.use_mxdog==1:
            ...
            l = np.repeat(np.array([[[[-8,-8,-8],[-8,1,-8],[-8,-8,-8],[-8,-8,-8],[-8,1,-8],[-8,-8,-8],[-8,-8,-8],[-8,1,-8],[-8,-8,-8]]]]),5,axis=0)
            print(l)
            print(l.shape)
            self.lap_filter = paddle.nn.Conv2D(1,1,(3,3),stride=1,bias_attr=False,
