...
class VectorQuantize(nn.Layer):
    def __init__(
        ...
        if transformer_size==1:
            ...
            self.transformer = Transformer(128, 4, 4, 128, 128, dropout=0.1)
            self.pos_embedding = paddle.create_parameter(shape=(1, 128, 128), dtype='float32')
    @property
    def codebook(self):
