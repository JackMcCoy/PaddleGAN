...
class LapStyleDraXDOG(BaseModel):
    ...
    def backward_Dec(self):
        ...
        self.loss = self.loss_c * self.content_weight + self.style_weight * (self.loss_s +3*self.loss_style_remd)+\
                    ...
                    mxdog_content * .3 + mxdog_content_contraint *100 + mxdog_content_img * 1000+\
                    self.loss_content_relt * 16
        self.loss.backward()
...
class LapStyleRevSecondMXDOG(BaseModel):
    ...
    def forward(self):
        ...
        if self.train_layer>3:
            ...
            stylized_rev_patch_second,_ = self.nets['net_rev_4'](revnet_input.detach(),stylized_feats,self.ada_alpha/3)
            stylized_rev_patch_second = fold_laplace_patch(
                [stylized_rev_patch_second, stylized_up.detach()])
            self.visual_items['ci_4'] = self.content_stack[3]
            self.visual_items['stylized_rev_fourth'] = stylized_rev_patch_second
    ...
    def train_iter(self, optimizers=None):
        ...
        for i in range(self.train_layer):
            optimizers[self.o[i]].clear_grad()
            self.set_requires_grad(self.nets[self.discriminators[i]],True)
            loss = self.backward_D(self.nets[self.discriminators[i]],i,str(i))
            loss.backward()
            optimizers[self.o[i]].step()
            self.set_requires_grad(self.nets[self.discriminators[i]],False)
            optimizers[self.o[i]].clear_grad()
        ...
        for i in range(self.train_layer):
            optimizers[self.go[i]].clear_grad()
            loss = self.backward_G(i)
            loss.backward()
            optimizers[self.go[i]].step()
            optimizers[self.go[i]].clear_grad()
