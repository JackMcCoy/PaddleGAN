...
def xdog(im, g, g2,morph_conv,gamma=.99, phi=200, eps=-.01  , k=1.6):
    ...
    for i in range(im.shape[1]):
        ...
        for j in range(im.shape[0]):
            ...
            morphed[j,i,:,:]= paddle.zeros_like(morphed[j,i,:,:])+(imdiff[j,i,:,:] > mean).astype('float32')*(morphed[j,i,:,:]>=mean).astype('float32')
    return morphed
                                ...
                                padding=4, padding_mode='reflect',
                                                weight_attr=paddle.ParamAttr(
                                                    initializer=paddle.fluid.initializer.NumpyArrayInitializer(
                                                        value=gaussian(9, 1).numpy()), trainable=False)
                                                )
        self.gaussian_filter_2 = paddle.nn.Conv2D(1, 1,9,
                                groups=1, bias_attr=False,
                                padding=4, padding_mode='reflect',
                                weight_attr = paddle.ParamAttr(
                                        initializer=paddle.fluid.initializer.NumpyArrayInitializer(value=gaussian(9, 1*1.6).numpy()), trainable=False)
                                    )

        self.morph_conv = paddle.nn.Conv2D(1,1,3,padding=1,groups=1,
                                           padding_mode='reflect',bias_attr=False,
                                           weight_attr = paddle.ParamAttr(
                                        initializer=paddle.fluid.initializer.Constant(value=1), trainable=False)
                                    )
