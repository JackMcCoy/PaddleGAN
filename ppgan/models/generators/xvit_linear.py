...
def look_around(x, backward = 1, forward = 0, pad_value = -1, dim = 2):
    ...
    padded_x = F.pad(x, [*dims[:-2],backward,forward], data_format='NCL' if len(tensor.shape)==3 else 'NCHW',value= pad_value)
...
class LocalAttention(nn.Layer):
    ...
    def forward(self, q, k, v, input_mask = None):
        ...
        a,b,c,d = bq_k.shape
        reshape_bq_k = paddle.reshape(bq_k,(a,b,1,c,d))
        if shared_qk:
        ...
        if input_mask is not None:
            ...
            a,b,c,d = mk.shape
            reshaped_mk = paddle.reshape(mk,(a,b,1,c,d))
