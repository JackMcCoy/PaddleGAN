...
def max_singular_value(W, u=None, Ip=1):
    ...
    print(W.shape)
    for _ in range(Ip):
        _v = _l2normalize(paddle.matmul(_u, paddle.transpose(W,[1,0])), eps=1e-12)
        _u = _l2normalize(paddle.matmul(_v, W), eps=1e-12)
...
class LapStyleSpectralDiscriminator(nn.Layer):
    def __init__(self, num_channels=32,num_layer=3,padding=1,kernel=3):
        ...
        for i in range(num_layer - 1):
            ...
            ndf=ndf*2
        self.relu = nn.Sequential(nn.ReLU(),nn.AvgPool2D(2),nn.AvgPool2D(2))
    ...
    def forward(self, x):
        ...
        x = paddle.reshape(x,(x.shape[0],1,256,256))
        return x