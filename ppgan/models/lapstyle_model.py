...
def xdog(im, g, g2,morph_conv,gamma=0.96, phi=50, eps=-0.1, k=1.6):
    ...
    for j in range(im.shape[0]):
        for i in range(im.shape[1]):
            imdiff[j,i,:,:] -= imdiff[j,i,:,:].min(axis=0)
            imdiff[j,i,:,:] /= imdiff[j,i,:,:].max(axis=0)
...
class LapStyleDraXDOG(BaseModel):
    def __init__(self,
        ...
        self.content_weight = content_weight
        self.style_weight = style_weight
        self.gaussian_filter = paddle.nn.Conv2D(1, 1,10,
                                groups=1, bias_attr=False,
                                weight_attr=paddle.ParamAttr(initializer=paddle.nn.initializer.Normal(mean=0,std=1),trainable=False),
                                padding=4, padding_mode='reflect')
        self.gaussian_filter_2 = paddle.nn.Conv2D(1, 1,10,
                                groups=1, bias_attr=False,
                                weight_attr=paddle.ParamAttr(initializer=paddle.nn.initializer.Normal(mean=0,std=1.8),trainable=False),
                                padding=4, padding_mode='reflect')
