...
class VectorQuantize(nn.Layer):
    def __init__(
        ...
        if transformer_size==1:
            self.transformer = Transformer(dim**2*2, 6, 4, dim**2*2, dim**2*2, dropout=0.1)
            self.pos_embedding = paddle.create_parameter(shape=(1, 256, 512), dtype='float32')
        elif transformer_size==2:
            self.transformer = Transformer(256, 4, 4, 256, 256, dropout=0.1)
    ...
    def forward(self, input):
        ...
        trans = self.rearrange(quantize)
        b, n, _ = trans.shape
        trans += self.pos_embedding[:, :n]
        trans = self.transformer(trans)
        trans = self.decompose_axis(trans)
        quantize += trans
