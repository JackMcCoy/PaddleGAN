...
class LapStyleRevSecondMXDOG(BaseModel):
    ...
    def backward_G(self,i):
        ...
        for j in range(i+1):
            self.cX = paddle.slice(self.cX,axes=[2,3],starts=[self.positions[j][0].astype('int32'),self.positions[j][1].astype('int32')],ends=[self.positions[j][2].astype('int32'),self.positions[j][3].astype('int32')])
